...
act_feat_basic = pkl.load(open(os.path.join(data_path, 'act_features.pkl'),'rb'))
user_cat_feat = [] #['gender', 'cluster_label']
user_num_feat = ['age','user_enroll_count']
course_cat_feat = ['course_category']
course_num_feat = ['course_enroll_count']

def feat_augment(train_feat, test_feat):
    act_feats = pkl.load(open(os.path.join(data_path, 'act_features.pkl'), 'rb'))
...
def load_data():
    ...
    dfTrain = pd.read_csv(os.path.join(data_path, 'train_normalized_features.csv'), index_col=0)
    dfTest = pd.read_csv(os.path.join(data_path, 'test_normalized_features.csv'), index_col=0)
